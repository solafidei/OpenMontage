...
| **VEO 3.1 (standard / fast / lite)** | `creative/prompting/veo-prompting.md` + [Vertex AI Prompt Guide](https://cloud.google.com/vertex-ai/generative-ai/docs/video/video-gen-prompt-guide) | Best vocabulary reference tables. 14-component prompt structure. Veo 2 / 3.0 shut down 2026-06-30 - only `veo-3.1-*` (Gemini API / Vertex) and `fal-ai/veo3.1`, `/fast`, `/lite` are live. 4/6/8 s, 16:9 or 9:16, native audio on by default, negative prompt, up to 3 reference images. `veo_video` defaults `resolution` to **1080p** (fal's own endpoint default is 720p), and 1080p / 4K (not lite) force duration to 8 s — silently coerced when `auto_fix` is left on — so pass `resolution="720p"` explicitly for a 4 s or 6 s clip. Google recommends Gemini Omni Flash by default; pick Veo 3.1 for last-frame control (`first_last_frame_to_video`) or legacy pipelines; `veo_video` exposes no extend operation. |
| **Gemini Omni Flash 1.1** | Layer 3 `.agents/skills/gemini-omni/` | Google's recommended default video model (`gemini-omni-1.1-flash`, GA 2026-08-27; fal `google/gemini-omni-flash/v1.1/*`). Tools: `gemini_omni_video` (native Gemini API; must target `gemini-omni-1.1-flash`) and `gemini_omni_fal` (calls the `google/gemini-omni-flash/v1.1/*` endpoints, billed per second of output by resolution). 3-10 s per generation (the model's `extend` task reaches 40 s in 3-10 s steps, but neither tool exposes it), 360p / 720p / 1080p / 4K, 16:9 or 9:16, synthesized audio, stateful conversational edits via `previous_interaction_id`, `<FIRST_FRAME>` / `<IMAGE_REF_N>` tags and `[0-3s]` timecodes. About $0.10/s at 720p. `gemini-omni-flash-preview` shuts down 2026-09-30. |
...
| **Kling 3 / O3 / 2.1 (fal.ai)** | [Kling 2.6 Pro Prompt Guide](https://fal.ai/learn/devs/kling-2-6-pro-prompt-guide) | 4-part structure. `++emphasis++` syntax is documented for 2.6. (Kling 2.6 is not a `kling_video` variant — the guide is kept for prompting technique only; the shipped variants are the v3/o3 lines plus legacy v2.1.) `kling_video` now defaults to `v3/standard`; fal also lists `v3/pro`, `v3/turbo/standard`, `v3/turbo/pro`, `o3/standard` and `o3/pro` (plus the 4K endpoints below) - 3-15 s, 16:9 / 9:16 / 1:1. Native audio: fal's endpoint default is on for v3 and off for o3, but `kling_video`'s own `generate_audio` default is `true` for every variant and the tool sends it explicitly (`tools/video/kling_video.py:154`, `:219-220`), so pass `generate_audio: false` to get the audio-off rate (5 s v3/standard: $0.42 off vs $0.63 on; 5 s o3/standard: $0.42 off vs $0.56 on). Per second: $0.084-0.168/s for the standard/pro/turbo tiers, $0.154-0.196/s with voice control (not exposed by `kling_video`), $0.42/s for `v3/4k` and `o3/4k` (a 5 s 4K clip is $2.10 - not to be confused with the $0.42 pinned 5 s v3/standard cutaway clip). The v2.1 line stays public as legacy — `v2.1/master`, `v2.1/pro`, `v2.1/standard`, 5 or 10 s only, and `v2.1/pro`/`v2.1/standard` exist on fal as image-to-video only. |
